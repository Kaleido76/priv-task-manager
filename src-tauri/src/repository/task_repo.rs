...
pub fn find_by_project_id(db: &Database, project_id: i64) -> Result<Vec<Task>, String> {
    ...
    let mut stmt = conn
        .prepare(
            "SELECT id, project_id, title, status, priority, recipient, deadline, create_time, update_time \
...
pub fn insert(db: &Database, task: &Task) -> Result<Task, String> {
    let conn = db.conn.lock().map_err(|e| e.to_string())?;
    conn.execute(
        "INSERT INTO tasks (project_id, title, status, priority, recipient, deadline, create_time, update_time) \
...
pub fn update(db: &Database, task: &Task) -> Result<Task, String> {
    ...
    conn.execute(
        "UPDATE tasks SET project_id = ?1, title = ?2, status = ?3, priority = ?4, recipient = ?5, \
         deadline = ?6, update_time = ?7 WHERE id = ?8",
        rusqlite::params![
            task.project_id,
            task.title,
            task.status,
